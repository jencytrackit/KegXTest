...
Namespace TrackITKTS
    Public Class dalTabMBranchPlant
        ...
        Public Shared Function GetTabMBranchPlantCompByBranchID(ByVal valBranchID As Int32, ByVal SchemaName As String) As Integer
            Dim db As DBAccess = New DBAccess
            Dim sqlCmd As SqlCommand = New SqlCommand()
            db.AddParamToSQLCmd(sqlCmd, "@ReturnValue", SqlDbType.VarChar, 200, ParameterDirection.ReturnValue, Nothing)
            db.AddParamToSQLCmd(sqlCmd, "@BranchID", SqlDbType.Int, 0, ParameterDirection.Input, valBranchID)
            db.AddParamToSQLCmd(sqlCmd, "@count", SqlDbType.VarChar, 3, ParameterDirection.Output, Nothing)
            db.SetCommandType(sqlCmd, CommandType.StoredProcedure, SchemaName + ".TIT_GetAllTabMBranchPlantCompByBranchID")
            db.ExecuteScalarCmd(sqlCmd)
            Dim returnValue As Integer = CInt(sqlCmd.Parameters("@count").Value)
            If Not String.IsNullOrEmpty(returnValue) Then
                Return returnValue
            Else
                Return 0
